...
namespace Autobarn.Website.Tests {
	public class ApiTests : IClassFixture<TestWebApplicationFactory<Startup>> {
        ...
        public async void GET_vehicles_returns_vehicle_data() {
            ...
            var resource = JsonConvert.DeserializeObject<dynamic>(json);
            var links = resource._links;
            var href = ((string) links.next.href);
            var nextResponse = await client.GetAsync(href);
            nextResponse.IsSuccessStatusCode.ShouldBe(true);
        }
        ...
        public async void POST_creates_vehicle() {
            ...
            var response = await client.PostAsync($"/api/models/volkswagen-beetle", content);
